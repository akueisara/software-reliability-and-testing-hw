...
token_stream tstream_ptr;
{
	  ...
	  } else if (next_st == -2) {statement[1]=1;/* This is numeric case. */
...
character_stream stream_ptr;
{
	...
	if(c==EOF){statement[5]=1; unget_char(c, stream_ptr);} /* Put back to leave gracefully - hf */
        return;
}
