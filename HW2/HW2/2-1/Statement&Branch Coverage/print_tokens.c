...
char argv[];
{
     ...
     // {
      ...
      //    exit(1);
     // }
      stream_ptr=open_token_stream(argv);
...
token_stream tstream_ptr;
{
	  ...
	  if (next_st == -1) {statement[8]=1; /* ERROR or EOF case */
	      return(error_or_eof_case(tstream_ptr, 
				       token_ptr,cu_state,token_str,token_ind,ch));
	  } else if (next_st == -2) {statement[9]=1;/* This is numeric case. */
	      return(numeric_case(tstream_ptr,token_ptr,ch,
				  token_str,token_ind));
	  } else if (next_st == -3) {statement[10]=1;/* This is the IDENTIFIER case */
...
character_stream stream_ptr;
{
	...
	if(c==EOF){statement[16]=1; unget_char(c, stream_ptr);} /* Put back to leave gracefully - hf */
        return;
}
...
char ch;
{
    if(state < 0){statement[17]=1;
      return(state);}
    if(base[state]+ch >= 0)
    {statement[18]=1;
        if(check[base[state]+ch] == state) /* Check for the right state */
         {statement[19]=1;    return(next[base[state]+ch]);}
        else{statement[20]=1;
...
token t;
{
    if(t->token_id==EOTSTREAM){statement[21]=1;
        return(TRUE);}
    return(FALSE);
}
...
char token_str[];
{
          ...
          for(ind=0;ind<token_ind;++ind)
                if(!isspace(token_str[ind])){statement[22]=1;
                      break;}
